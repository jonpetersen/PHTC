...
if (!class_exists('EasyFacebookPostFeed')){
	class EasyFacebookPostFeed {			
			...
			public function displayFeed()
			{
				...
				$date     	= get_option('EasyFacebookPostFeed_display_date');
				...
				$align 		= get_option('EasyFacebookPostFeed_display_align');
				...
				if($err == '')
				{
					...
					if (isset($fbApiGetPosts["data"]) && !empty($fbApiGetPosts["data"])) {
					    echo '<p style="height:1px; padding:0; margin:0; text-indent:-9999999px;">' . __('') . ': <a href="http://www.marioconcina.it/blog">Mario Concina</a>';
					    ...
					    foreach($fbApiGetPosts["data"] as $data)
					    {
					    	if($data['message'] != '')
					    	{
					    		switch($align)
					    		{
					    			case 'left': $alignment = 'float:left; margin-right:15px;'; break;
					    			case 'below': $alignment = null; break;
					    			case 'right': $alignment = 'float:right; margin-left:15px;'; break;
					    		}
					    		list($anno, $mese, $resto) = explode('-', $data['created_time']);
					    		list($giorno, $ora) = explode('T', $resto);
						    	echo '<li><p align="left">';
						    	if($date == 'yes') echo $giorno . '/' . $mese . '/' . $anno . '<br />';
